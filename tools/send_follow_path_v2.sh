...
docker exec "${CONTAINER}" bash -lc "export RMW_IMPLEMENTATION=${RMW_IMPLEMENTATION_VALUE} && source /opt/ros/humble/setup.bash && source /ros2_ws/install/setup.bash && ros2 action send_goal /follow_path nav2_msgs/action/FollowPath \"{path: {header: {frame_id: odom}, poses: [{header: {frame_id: odom}, pose: {position: {x: ${START_X_M}, y: ${START_Y_M}, z: 0.0}, orientation: {x: 0.0, y: 0.0, z: 0.0, w: 1.0}}}, {header: {frame_id: odom}, pose: {position: {x: ${MID_X_M}, y: ${START_Y_M}, z: 0.0}, orientation: {x: 0.0, y: 0.0, z: 0.0, w: 1.0}}}, {header: {frame_id: odom}, pose: {position: {x: ${END_X_M}, y: ${TARGET_Y_M}, z: 0.0}, orientation: {x: 0.0, y: 0.0, z: 0.0, w: 1.0}}}]}, controller_id: 'FollowPath', goal_checker_id: 'general_goal_checker'}\" --feedback"
