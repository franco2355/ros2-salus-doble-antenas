...
def generate_launch_description():
    ...
    vision_pipeline_dir = get_package_share_directory("vision_pipeline")
    lidar_camara_dir = get_package_share_directory("lidar_camara")
    ...
    use_ip_camera = LaunchConfiguration("use_ip_camera")
    vision_model_path = LaunchConfiguration("vision_model_path")
    camera_stream_url = LaunchConfiguration("camera_stream_url")
    camera_device = LaunchConfiguration("camera_device")
    perception_enable_recovery = LaunchConfiguration("perception_enable_recovery")
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "enable_perception",
                default_value="false",
                description="Start LIDAR+camera fusion pipeline (vision_pipeline + lidar_camara).",
            ),
            DeclareLaunchArgument(
                "use_ip_camera",
                default_value="false",
                description="Use IP camera (RTSP) instead of USB camera for perception.",
            ),
            DeclareLaunchArgument(
                "vision_model_path",
                default_value="",
                description="Absolute path to YOLO ONNX model. Required when enable_perception:=true.",
            ),
            DeclareLaunchArgument(
                "camera_stream_url",
                default_value="",
                description="RTSP/MJPEG URL when use_ip_camera:=true.",
            ),
            DeclareLaunchArgument(
                "camera_device",
                default_value="/dev/video0",
                description="V4L2 device when use_ip_camera:=false.",
            ),
            DeclareLaunchArgument(
                "perception_enable_recovery",
                default_value="true",
                description=(
                    "Enable autonomous obstacle recovery (brake → backup → resume Nav2). "
                    "Requires enable_perception:=true."
                ),
            ),
            ...
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    os.path.join(vision_pipeline_dir, "launch", "vision_pipeline.launch.py")
                ),
                launch_arguments={
                ...
                condition=IfCondition(
                    PythonExpression(
                        ["'", enable_perception, "'.lower() == 'true' and '",
                         use_ip_camera, "'.lower() != 'true'"]
                    )
                ),
            ),

            # ── Percepción: pipeline IP (cámara RTSP + YOLO) ────────────────
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    os.path.join(vision_pipeline_dir, "launch", "vision_pc_gpu.launch.py")
                ),
                launch_arguments={
                    "model_path": vision_model_path,
            ...
            # ── Fusión LIDAR + cámara (fusion_node + obstacle_recovery) ─────
            IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    os.path.join(lidar_camara_dir, "launch", "lidar_camara.launch.py")
                ),
                launch_arguments={
                    "enable_recovery": perception_enable_recovery,
                }.items(),
