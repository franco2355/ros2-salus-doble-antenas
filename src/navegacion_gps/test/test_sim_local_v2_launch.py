...
def test_sim_local_v2_launch_uses_realistic_command_chain() -> None:
    ...
    assert 'load_navigation_profile(navigation_profiles_file, "local_v2")' in launch_contents
    ...
    assert 'DeclareLaunchArgument("map_frame", default_value=local_profile.map_frame)' in launch_contents
    assert 'DeclareLaunchArgument("fromll_frame", default_value=local_profile.fromll_frame)' in launch_contents
